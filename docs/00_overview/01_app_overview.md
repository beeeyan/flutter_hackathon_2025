...
**名称：** Myacle  
**読み：** ミャクる  
**名付けにこめた思い：** 「脈」+「miracle」 奇跡の出会い、「脈来る」 etc.  
...
- **ペア成立発表:** お互いに一定以上のタップ数を送り合ったペア（脈アリ）を画面に表示する。表示されるのは、マッチしたユーザーの画面のみ。マッチしなかった場合は「今回はマッチしませんでした」などを表示。
...
→ Cloud Functionsは利用せず、端末ごとでの測定とし、`startTime` はスキーマからも削除  