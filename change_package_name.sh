...
if [ $# != 1 ]; then
    ...
    sed -i "" -E "s/applicationId = \".*\"/applicationId = \"$1\"/g" ./android/app/build.gradle.kts
    sed -i "" -E "s/namespace = \".*\"/namespace = \"$1\"/g" ./android/app/build.gradle.kts
    echo build.gradle.ktsの変更完了
